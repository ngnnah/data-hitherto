...
def process_regrid_chunk(stem, chunk_order, chunk_df):
    ...
    # ERROR: pyarrow.lib.ArrowTypeError: ("Expected bytes, got a 'list' object", 'Conversion failed for column property_name with type object')
    # HOTFIX? COERCE to str type
    df = pd.json_normalize(chunk_df.properties).astype(str) 
...
def process_sf(sf):
    sf = sf.lower()
    # NOTE THE HARD-CODED FILE!
    files = glob(f'/home/nhat/regrid-bucket/{sf}_dallas.ndgeojson.gz')
