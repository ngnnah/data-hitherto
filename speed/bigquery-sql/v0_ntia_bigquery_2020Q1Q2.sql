...
# Original NTIA query ends with the subquery below as the final selection
# Modified as a subquery here to calculate test counts
--Show the results
NTIA_results AS (
SELECT 
  MIN(download_MIN) AS download_MIN,
  AVG(download_Q10) as download_Q10,
  --PERCENTILE_CONT(MAX(download_MAX),0.5) OVER() AS download_MED,
  AVG(download_MED) as download_MED,
  AVG(download_Q90) AS download_Q90,
  MAX(download_MAX) AS download_MAX,
  MIN(upload_MIN) AS upload_MIN,
  AVG(upload_Q10) AS upload_Q10,
  --PERCENTILE_CONT(MAX(upload_MAX),0.5) OVER() AS upload_MED,
  AVG(upload_MED) AS upload_MED, 
  AVG(upload_Q90) AS upload_Q90,
  MAX(upload_MAX) AS upload_MAX,
  county_fips_code
FROM results
GROUP BY county_fips_code
),
# Count the number of daily tests within the NTIA us counties query
ndt_per_day_cnt_us_counties AS (
  SELECT date, dl_samples_day, ul_samples_day
  FROM results
  GROUP BY date, dl_samples_day, ul_samples_day
)
# SUM the daily counts for us counties
SELECT SUM(dl_samples_day) AS cnt_ndt_down, SUM(ul_samples_day) AS cnt_ndt_up
FROM ndt_per_day_cnt_us_counties
